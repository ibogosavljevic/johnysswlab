...
void cos_vector(double* in, double* out, size_t n) {
    constexpr double tp_scalar = 1. / (2. * M_PI);
...
void __attribute__((noinline)) run_test_longchain(const std::vector<double> v, int cnt1, int cnt2)
{
    ...
    std::string name4 = name + "_vec_fiss";
    sum_vec = set_vec(0.0);

    std::vector<double_vec> tmp_vec(cnt1);

    LIKWID_MARKER_START(name4.c_str());
    ...
    if (cnt2 > 1) {
        ...
        for (int i = 0; i < cnt1; i += VECTOR_SIZE) {
            sum_vec += cos_vector(load_vec(v_ptr + i));
        }
    }

    LIKWID_MARKER_STOP(name4.c_str());

    constexpr int BLOCK_SIZE = 128;

    std::cout << "sum_vec_fiss = " << horizontal_add(sum_vec) << std::endl;

    std::string name5 = name + "_vec_fiss_block";
    sum_vec = set_vec(0.0);

    std::vector<double_vec> tmp_vec2(BLOCK_SIZE);

    LIKWID_MARKER_START(name5.c_str());
    ...
    LIKWID_MARKER_STOP(name5.c_str());

    std::cout << "sum_vec_fiss_block = " << horizontal_add(sum_vec) << std::endl;
