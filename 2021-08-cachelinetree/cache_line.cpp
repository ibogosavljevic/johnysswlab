...
template <int len>
void measure_time(const std::vector<int>& input, const std::vector<int>& search_data) {
    ...
    {
        class measure_time m(std::string("measure len ") + std::to_string(len));
...
int main(int argc, char** argv) {
    std::vector<int> sorted_data = create_random_array<int>(ARR_LEN, 0, ARR_LEN);
    std::sort(sorted_data.begin(), sorted_data.end());
    auto it = std::unique(sorted_data.begin(), sorted_data.end());
    sorted_data.resize(std::distance(sorted_data.begin(), it));

    std::vector<int> search_data = create_random_array<int>(ARR_LEN, -10, ARR_LEN + 10);

    measure_time<1>(sorted_data, search_data);
    measure_time<2>(sorted_data, search_data);
    measure_time<3>(sorted_data, search_data);
    measure_time<4>(sorted_data, search_data);
    measure_time<5>(sorted_data, search_data);
    measure_time<6>(sorted_data, search_data);
    measure_time<7>(sorted_data, search_data);
    measure_time<8>(sorted_data, search_data);
}