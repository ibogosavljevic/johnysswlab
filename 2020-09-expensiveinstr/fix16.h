...
static const fix16_t fix16_minimum =
    0x80000000; /*!< the minimum value of fix16_t */
static const fix16_t fix16_overflow =
    0x80000000; /*!< the value used to indicate overflows when
...
/*! Performs a saturated multiplication (overflow-protected) of the two given
 * fix16_t's and returns the result.
 */
extern fix16_t fix16_smul(fix16_t inArg0, fix16_t inArg1) FIXMATH_FUNC_ATTRS;

/*! Performs a saturated division (overflow-protected) of the first fix16_t by
...
/*! Returns the linear interpolation: (inArg0 * (1 - inFract)) + (inArg1 *
 * inFract)
 */
extern fix16_t fix16_lerp8(fix16_t inArg0,
                           fix16_t inArg1,
                           uint8_t inFract) FIXMATH_FUNC_ATTRS;
extern fix16_t fix16_lerp16(fix16_t inArg0,
                            fix16_t inArg1,
                            uint16_t inFract) FIXMATH_FUNC_ATTRS;
#ifndef FIXMATH_NO_64BIT
extern fix16_t fix16_lerp32(fix16_t inArg0,
                            fix16_t inArg1,
...
#define FIXMATH_CONVERT_MANTISSA(m)                                           \
    ((unsigned)((((uint64_t)(((1##m##ULL) -                                   \
                              FIXMATH_CONSTANT_POW10(FIXMATH_TOKLEN(m))) *    \
...
#define FIXMATH_COMBINE_I_M(i, m) \
    (((i) << 16) | (FIXMATH_CONVERT_MANTISSA(m) & 0xFFFF))
 ...
 *
 * @note Regardless of the specified minimum and maximum values for i and m
 * below, the total value of the number represented by i and m MUST be in the
 * interval
 *       ]-32768.00000:32767.99999[ else usage with this macro will yield
 * undefined behavior.
 *
 * @param i Signed integer constant with a value in the interval ]-32768:32767[.
 * @param m Positive integer constant in the interval ]0:99999[ (fractional
 * part/mantissa).
 */
#define F16C(i, m)                                                   \
    ((fix16_t)(((#i[0]) == '-')                                      \
