...
result_t calculate_pi_vector3(double convergence_diff) {
    __m256d sum = _mm256_set1_pd(0.0);

    const __m256d convergence_diff_v = _mm256_set1_pd(convergence_diff);
    ...
    do {
        ...
        x = _mm256_add_pd(x1, x2);
        sum = _mm256_add_pd(sum, x);

    } while (_mm256_movemask_pd(_mm256_cmp_pd(x, convergence_diff_v, _CMP_LE_OS)) == 0);
...
int main(int argc, char** argv) {
    ...
    LIKWID_MARKER_START("vector3_pi1");
    result_t pi_vector31 = calculate_pi_vector3(1e-18);
    LIKWID_MARKER_STOP("vector3_pi1");

    LIKWID_MARKER_START("vector3_pi2");
    result_t pi_vector32 = calculate_pi_vector3(1e-17);
    LIKWID_MARKER_STOP("vector3_pi2");

    LIKWID_MARKER_START("vector3_pi3");
    result_t pi_vector33 = calculate_pi_vector3(4e-17);
    LIKWID_MARKER_STOP("vector3_pi3");

    LIKWID_MARKER_START("vector1_pi1");
    result_t pi_vector11 = calculate_pi_vector1(4e-17);
    LIKWID_MARKER_STOP("vector1_pi1");
    ...
    std::cout << "vector3_1 diff " << std::abs(pi4 - pi_vector31.pi_4) << "\n";
    std::cout << "vector3_2 diff " << std::abs(pi4 - pi_vector32.pi_4) << "\n";
    std::cout << "vector3_3 diff " << std::abs(pi4 - pi_vector33.pi_4) << "\n";
    std::cout << "vector1 diff " << std::abs(pi4 - pi_vector11.pi_4) << "\n";
    std::cout << "vector2 diff " << std::abs(pi4 - pi_vector21.pi_4) << "\n";

    std::cout << "compiler last member " << pi_compiler.last_member << "\n";
    std::cout << "vector3_1 last member " << pi_vector31.last_member << "\n";
    std::cout << "vector3_2 last member " << pi_vector32.last_member << "\n";
    std::cout << "vector3_3 last member " << pi_vector33.last_member << "\n";
    std::cout << "vector1 last member " << pi_vector11.last_member << "\n";
    std::cout << "vector2 last member " << pi_vector21.last_member << "\n";
